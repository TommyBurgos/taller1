...
class Plane:
    ...
    def sum(self):
        validThis = self.myclass.validThis(self.dist)
        validNumber = self.passanger.validNumber()
        validTime = self.total_TIME.is_valid_total_TIME()
        if not validThis or not validNumber or not validTime:
            return -1
...
class Vacation_:
    ...
    def sum(self):
        ...
        validThis = self.myclass.validThis(self.dist)
        validNumber = self.passagner.validNumber()
        validTime = self.total_TIME.is_valid_total_TIME()
        if not validThis or not validNumber or not validTime:
            return -1
        # sum the total cost
